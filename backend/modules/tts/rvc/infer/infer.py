...
def warmup_rvc_runtime(
    *,
    ...
    vc = get_vc(model_path, 0, None, 0, debug_rvc)
    load_hubert(embedder_model)
    vc.ensure_f0_method_ready(f0_method)

    # Warm lightweight runtime path so the first real conversion does not pay
    # all initialization costs on the hot path.
    normalized_method = str(f0_method or "").strip().lower()
    if normalized_method in {"crepe", "crepe-tiny"}:
        sample = np.zeros(vc.sr, dtype=np.float32)
        p_len = max(1, sample.shape[0] // vc.window)
        vc.get_f0(
            "warmup",
            sample,
            p_len,
    ...
    return {
        ...
        "f0_method_ready": True,
        "loaded_model_file": os.path.basename(model_path),
    }
...
def get_vc(weight_root, sid, file_index=None, training_data_size=10000, debug_rvc=False):
    ...
    if debug_rvc:
        print(f"[{branding}Debug] Index support is disabled in this build")
    data = None
    d = None
    nlist = None
    train_data = None
