...
import pyworld, os, librosa, torchcrepe
...
RVC_BASE_ROOT = Path(RVC_MODELS_DIR) / "rvc_base"
...
def normalize_pitch_audio(x, quantile: float = 0.999, eps: float = 1e-6):
    ...
    scale = float(np.quantile(magnitude, quantile))
...
class VC(object):
    ...
    def _ensure_rmvpe_model(self):
        if hasattr(self, "model_rmvpe") is False:
            from ..lib.rmvpe import RMVPE

            model_path = str(RVC_BASE_ROOT / "rmvpe.pt")
            self.model_rmvpe = RMVPE(
                model_path, is_half=self.is_half, device=self.device
            )
        return self.model_rmvpe

    def _ensure_fcpe_model(self, f0_min: int = 50, f0_max: int = 1100):
        if hasattr(self, "model_fcpe") is False:
            model_path = str(RVC_BASE_ROOT / "fcpe.pt")
    ...
    def ensure_f0_method_ready(self, f0_method: str):
        method = str(f0_method or "").strip().lower()
        if method == "rmvpe":
            self._ensure_rmvpe_model()
            return True
        if method == "fcpe":
            self._ensure_fcpe_model()
            return True
        if method in {"pm", "dio", "crepe", "crepe-tiny"}:
            return True
        if "hybrid" in method:
            return True
        raise ValueError(f"Unsupported RVC F0 method: {f0_method}")
    ...
    def get_f0_hybrid_computation(
        ...
        for method in methods:
            ...
            if method == "crepe":
                ...
                f0 = self._ensure_rmvpe_model().infer_from_audio(x, thred=0.03)
                f0 = f0[1:]
            elif method == "fcpe":
                f0 = self._ensure_fcpe_model(f0_min, f0_max).compute_f0(x, p_len=p_len)
