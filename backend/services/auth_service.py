...
def _serialize_user(user: User) -> dict:
    ...
    return {
        ...
        "created_at": (
            _as_utc(user.created_at).isoformat()
            if user.created_at
            else None
        ),
        "last_login_at": (
            _as_utc(user.last_login_at).isoformat()
...
def register_user(
    *,
    ...
    try:
        existing_auth_users = (
            session.query(User).filter(User.password_hash.isnot(None)).count()
        )
...
def get_user_from_access_token(token: str) -> Optional[User]:
    ...
    try:
        ...
        if not user or not user.is_active:
            return None
        # Ensure settings relation is loaded before session closes.
        _ = user.settings
        return user
    finally:
        session.close()
...
def get_auth_bootstrap_state() -> dict:
    """
    Returns auth bootstrap state used by frontend guards:
    - first valid account must be owner
    - anonymous mode is allowed only after owner exists
    """
    session: Session = SessionLocal()
    try:
        auth_users_count = (
        ...
        # Fresh DB or schema not ready yet.
        auth_users_count = 0
        has_owner = False
    finally:
        session.close()
